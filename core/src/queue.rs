...
pub mod tests {
    ...
    async fn queue_throttling() {
        ...
        let world = {
            ...
            let mut alice_account = Account::new(alice_id.clone()).build(&alice_id);
            alice_account.activate();
            let mut bob_account = Account::new(bob_id.clone()).build(&bob_id);
            bob_account.activate();
