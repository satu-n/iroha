...
case $1 in
    "genesis")
        cargo run --release --bin kagami -- genesis generate --executor executor.wasm --wasm-dir ../wasm/target/prebuilt/libs/ --genesis-public-key ed01204164BF554923ECE1FD412D241036D863A6AE430476C898248B8237D77534CFC4 | diff - defaults/genesis.json || {
            echo 'Please re-generate the default genesis with `cargo run --release --bin kagami -- genesis --executor executor.wasm --wasm-dir ../wasm/target/prebuilt/libs/ --genesis-public-key ed01204164BF554923ECE1FD412D241036D863A6AE430476C898248B8237D77534CFC4 > ./defaults/genesis.json`'
